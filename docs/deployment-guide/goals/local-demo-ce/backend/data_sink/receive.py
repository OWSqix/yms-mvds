...
@router.post("/public/receive-file")
async def receive_file_public(
        request: Request
):
    """Public file receive endpoint without authentication - USE WITH CAUTION"""
    logger.debug("Public file receive request - NO AUTHENTICATION")
    logger.debug(f"All headers: {dict(request.headers)}")
    ...
    if not filename:
        ...
        filename = f"edc_public_{timestamp}_{unique_id}.{extension}"
    ...
    try:
        ...
        with open(save_path, "wb") as f:
            f.write(body)
        logger.info(f"Public file saved in: {save_path}")
    except Exception as e:
        logger.error(f"Failed to save public file: {e}")
        raise HTTPException(status_code=500, detail=str(e))

    return {
        "msg": "File is successfully transferred without authentication",
        "size": len(body),
        "filename": filename,
        "content_type": content_type
    }