...
public class EdcClientFactory {
    public static EdcClient newClient(EdcClientBuilder builder) {
        var apiClient = new ApiClient().setServerIndex(null).setBasePath(builder.managementApiUrl());
        ...
        if (builder.oauth2ClientCredentials() != null) {
            ...
            var httpClient = apiClient.getHttpClient().newBuilder().addInterceptor(new OAuth2CredentialsInterceptor(handler)).authenticator(new OAuth2CredentialsAuthenticator(handler)).build();
            apiClient.setHttpClient(httpClient);
        }
        if (builder.customConfigurer() != null) {
            builder.customConfigurer().accept(apiClient);
        }
        return new EdcClient(new UiApi(apiClient), new UseCaseApi(apiClient), new EnterpriseEditionApi(apiClient));
    }

    private EdcClientFactory() {
    }
}
