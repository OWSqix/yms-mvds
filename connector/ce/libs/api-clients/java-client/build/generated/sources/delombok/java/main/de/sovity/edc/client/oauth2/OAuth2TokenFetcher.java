...
public class OAuth2TokenFetcher {
    ...
    public OAuth2TokenResponse fetchToken() {
        try {
            var formData = new FormBody.Builder().add("grant_type", "client_credentials").add("client_id", clientCredentials.getClientId()).add("client_secret", clientCredentials.getClientSecret()).build();
            var request = new Request.Builder().url(clientCredentials.getTokenUrl()).post(formData).build();
    ...
    public OAuth2TokenFetcher(final OAuth2ClientCredentials clientCredentials) {
